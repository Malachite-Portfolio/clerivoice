...
import {
  getCallStatusMessageByCode,
  getCallStatusMessageFromError,
} from "../services/callStatusMessage";
...
const HomeScreen = ({ navigation }) => {
  ...
  const inboxQuery = useQuery({
    ...
    queryFn: () =>
      getInboxItems({ currentUserId: session?.user?.id, limit: 12 }),
    staleTime: 8000,
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeRealtimeSocketState(({ state }) =>
      setSocketState(state || SYNC.disconnected),
    );
    ...
    ].forEach((eventName) => socket.on(eventName, invalidateHosts));
    ["wallet_updated"].forEach((eventName) =>
      socket.on(eventName, invalidateWallet),
    );
    [
      "chat_started",
      "chat_accepted",
      ...
      "call_accepted",
      "call_ended",
    ].forEach((eventName) => socket.on(eventName, invalidateSessions));
    return () => {
      [
        "host_updated",
        "host_deleted",
        ...
        "referral_updated",
        "listener_status_changed",
      ].forEach((eventName) => socket.off(eventName, invalidateHosts));
      ["wallet_updated"].forEach((eventName) =>
        socket.off(eventName, invalidateWallet),
      );
      [
        "chat_started",
        "chat_accepted",
        ...
        "call_accepted",
        "call_ended",
      ].forEach((eventName) => socket.off(eventName, invalidateSessions));
      unsubscribe();
    };
    ...
    () =>
      (hostsQuery.data?.items || [])
        .filter((item) => {
          ...
          const status = up(
            item?.status || item?.accountStatus || item?.user?.status,
          );
        ...
        .map((item) => {
          ...
          const availability = normalizePresenceStatus(
            item.availability || PRESENCE_STATUS.OFFLINE,
          );
          ...
          return {
            ...
            availability,
            isOnline: availability === PRESENCE_STATUS.ONLINE,
  ...
  const filteredHosts = useMemo(() => {
    ...
    return hosts.filter((item) => {
      const searchableValue =
        `${item?.name || ""} ${item?.quote || ""} ${item?.experience || ""} ${item?.price || ""}`
          .trim()
          .toLowerCase();
  ...
  const filteredInboxItems = useMemo(() => {
    ...
    return inboxItems.filter((item) => {
      const searchableValue =
        `${item?.participant?.name || ""} ${item?.preview || ""} ${item?.type || ""} ${item?.status || ""}`
          .trim()
          .toLowerCase();
      return searchableValue.includes(normalizedSearchQuery);
    });
  ...
  const syncUi = {
    connected: {
      title: "LIVE SYNC ON",
      subtitle: "Connected and updating live data",
      button: "LIVE",
      helper: "",
      pill: styles.livePill,
    },
    reconnecting: {
      title: "LIVE SYNC RECONNECTING",
      subtitle: "Trying to restore live data",
      button: "SYNCING",
      helper: "Trying to restore live data",
      pill: styles.syncingPill,
    },
    disconnected: {
      title: "LIVE SYNC OFF",
      subtitle: "Live updates are temporarily unavailable",
      button: "RETRY",
      helper: "Live updates are temporarily unavailable",
      pill: styles.retryPill,
    },
  }[
    socketState === SYNC.connected
      ? "connected"
      : socketState === SYNC.reconnecting
        ? "reconnecting"
        : "disconnected"
  ];
  ...
  const validateHostAvailability = useCallback(async (listenerId) => {
    ...
    if (normalizedAvailability !== "ONLINE" || !isVisible || !isEnabled) {
      const reasonCode =
        normalizedAvailability === "BUSY" ? "HOST_BUSY" : "HOST_OFFLINE";
  ...
  const openCall = async (host) => {
    ...
    try {
      ...
      if (blocked) {
        Alert.alert(
          "Blocked contact",
          "Unblock this host from chat menu before starting calls.",
        );
        return;
      }
  ...
  const openChat = async (host) => {
    ...
    try {
      ...
      if (blocked) {
        Alert.alert(
          "Blocked contact",
          "Unblock this host from chat menu before starting a chat.",
        );
        return;
      }
      ...
      if (cachedActiveSession?.id) {
        navigation.navigate("ChatSession", {
          chatPayload: { session: cachedActiveSession, agora: null },
          host,
        });
        return;
      }
      ...
      if (existingActive?.id) {
        navigation.navigate("ChatSession", {
          chatPayload: { session: existingActive, agora: null },
          host,
        });
        return;
      }
  ...
  const openInboxItem = async (item) => {
    ...
    if (item?.type !== "chat") {
      return Alert.alert(
        "Call history",
        item?.preview || "Call activity updated.",
      );
    }
    ...
    const host = {
      ...
      availability: normalizePresenceStatus(
        item?.participant?.availability ||
          (up(item?.status) === "ACTIVE"
            ? PRESENCE_STATUS.ONLINE
            : PRESENCE_STATUS.OFFLINE),
      ),
    };
    host.isOnline = isPresenceOnline(host.availability);
    ...
    if (!isListenerRole && normalizedStatus !== "ACTIVE") {
      if (!host?.listenerId) {
        Alert.alert(
          "Chat unavailable",
          "Unable to start a new chat session right now.",
        );
        return;
      }
    ...
    navigation.navigate("ChatSession", {
      chatPayload: { session: item.session, agora: null },
      host,
    });
  };
        ...
        >
          <View style={styles.header}>
            ...
            <View style={styles.headerRight}>
              ...
              <WalletPill
                amount={walletQuery.data?.balance ?? currentBalance ?? 0}
                onPress={openWallet}
              />
            </View>
          </View>
          ...
          <View style={styles.searchBar}>
            ...
            <TextInput
              ...
              placeholder={
                activeTab === "Inbox" ? "Search inbox..." : "Search hosts..."
              }
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (session?.accessToken && socketState !== SYNC.connected)
                connectRealtimeSocket(session.accessToken);
              refreshLiveData();
            }}
            activeOpacity={0.9}
          >
            ...
            <View style={[styles.syncPill, syncUi.pill]}>
              <Text style={styles.syncPillText}>{syncUi.button}</Text>
            </View>
          </TouchableOpacity>
          {syncUi.helper ? (
            <View style={styles.syncHelper}>
              <Text style={styles.syncHelperText}>{syncUi.helper}</Text>
            </View>
          ) : null}
          ...
          {activeTab === "Verified" ? (
            <>
              ...
              >
                {filteredHosts.slice(0, 8).map((story) => (
                  <StoryAvatar
                    ...
                    presenceStatus={story.availability}
                    image={story.avatar}
              ...
              {hostsQuery.isLoading && !hostsQuery.data ? (
                <View style={styles.stateCard}>
                  <Text style={styles.stateText}>Loading live hosts...</Text>
                </View>
              ) : null}
                ...
                ? filteredHosts
                    ...
                    .map((person) => (
                      <SupportCard
                        key={person.id}
                        person={person}
                        talkDisabled={!isPresenceOnline(person.availability)}
                        chatDisabled={!isPresenceOnline(person.availability)}
            ...
            <View style={styles.inboxWrap}>
              ...
              {inboxQuery.isError && !inboxQuery.data ? (
                <View style={styles.errorCard}>
                  <Text style={styles.errorTitle}>Unable to load inbox.</Text>
                  <TouchableOpacity
                    style={styles.smallBtn}
                    onPress={refreshLiveData}
                  >
                ...
                <View style={styles.emptyCard}>
                  ...
                  <Text style={styles.emptySub}>
                    {normalizedSearchQuery
                      ...
                      : "Your real chat and call history will appear here once you connect with a host."}
                  </Text>
                </View>
              ) : null}
                  ...
                  <View style={styles.inboxAvatarWrap}>
                    <Image
                      source={resolveAvatarSource({
                        profileImageUrl:
                          item?.participant?.profileImageUrl || null,
                        id:
                          item?.participant?.id ||
                          item?.session?.listenerId ||
                          null,
                        userId:
                          item?.participant?.id ||
                          item?.session?.listenerId ||
                    ...
                    <View
                      style={[
                        styles.dot,
                        {
                          backgroundColor: getPresenceDotColor(
                            item?.participant?.availability ||
                              (up(item?.status) === "ACTIVE"
                                ? PRESENCE_STATUS.ONLINE
                                : PRESENCE_STATUS.OFFLINE),
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={styles.inboxBottom}>
                      <Text
                        style={[
                          styles.inboxPreview,
                          item?.unreadCount ? styles.inboxPreviewUnread : null,
                        ]}
                        numberOfLines={2}
                      >
                        {item?.preview || "Start conversation..."}
                      </Text>
                      {item?.unreadCount ? (
...
const styles = StyleSheet.create({
  ...
  demoBadge: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: "rgba(255,42,163,0.3)",
  ...
  syncHelperText: { color: theme.colors.textSecondary, fontSize: 12 },
  tabs: {
    flexDirection: "row",
    gap: 20,
    borderBottomWidth: 1,
  ...
  storyRow: { paddingTop: 10, paddingBottom: 8 },
  anonCard: {
    marginTop: 8,
    marginBottom: 12,
    borderRadius: 20,
  ...
  anonSub: { color: theme.colors.textSecondary, fontSize: 12, marginTop: 1 },
  sectionTitle: {
    color: theme.colors.textPrimary,
    fontSize: 17,
    fontWeight: "700",
  ...
  stateText: { color: theme.colors.textSecondary, fontSize: 14 },
  errorCard: {
    borderRadius: 18,
    borderWidth: 1,
  ...
  inboxWrap: { marginTop: 14, gap: 12 },
  emptyCard: {
    borderRadius: 24,
    borderWidth: 1,
    borderColor: "rgba(255,42,163,0.25)",
    backgroundColor: "rgba(24,13,35,0.92)",
  ...
  inboxAvatarWrap: { marginRight: 12 },
  inboxAvatar: {
    width: 60,
    height: 60,
    borderRadius: 30,
  ...
  dotOff: { backgroundColor: "rgba(255,255,255,0.26)" },
  inboxTop: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "flex-start",
  ...
  inboxBottom: { flexDirection: "row", alignItems: "center", marginTop: 6 },
  inboxName: {
    flex: 1,
    color: theme.colors.textPrimary,
    fontSize: 17,
  ...
  inboxTime: { color: theme.colors.textMuted, fontSize: 12, marginTop: 2 },
  inboxPreview: {
    flex: 1,
    paddingRight: 12,
    color: theme.colors.textSecondary,
  ...
  inboxPreviewUnread: { color: theme.colors.textPrimary },
  badge: {
    minWidth: 24,
    height: 24,
    borderRadius: 12,
    ...
    alignItems: "center",
    justifyContent: "center",
    backgroundColor: "rgba(255,42,163,0.92)",
  },
  ...
  chatPill: {
    ...
    backgroundColor: "rgba(15,214,122,0.12)",
  },
  callPill: {
