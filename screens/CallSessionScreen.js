...
const isSpeakerRoute = (routing) => {
  ...
  return (
    normalizedRouting === "speaker" || normalizedRouting === "speakerphone"
  );
};
...
const CallSessionScreen = ({ navigation, route }) => {
  ...
  const isDemoSession = Boolean(
    authSession?.isDemoUser || payload?.demoMode || incomingRequest?.demoMode,
  );
  ...
  const [localPreviewSurfaceVersion, setLocalPreviewSurfaceVersion] =
    useState(0);
  ...
  const callStartedAtRef = useRef(
    payload?.session?.startedAt || payload?.session?.answeredAt || null,
  );
  ...
  const displayName = useMemo(
    () =>
      host?.name || incomingRequest?.requester?.displayName || "Support Host",
    [host?.name, incomingRequest?.requester?.displayName],
  );
  ...
  const callMode =
    incomingRequest && !runtimePayload
      ? "incoming"
      : isCallConnected
        ? "active"
        : "outgoing";
  const showVideoSurfaces =
    isVideoCall && callMode !== "incoming" && !isDemoSession;
  ...
  useEffect(() => {
    logCallDebug("rendererMounted", {
      ...
      nativeLoadError: hasAgoraRtcSurfaceView
        ? null
        : agoraRtcUiLoadError?.message || null,
      hasLocalVideoFrame,
      isLocalPreviewBound,
      hasRemoteVideoFrame,
      remoteVideoUid,
  ...
  useEffect(() => {
    ...
    backendMarkedActiveRef.current =
      runtimePayloadRef.current?.session?.status === "ACTIVE";
    callStartedAtRef.current =
  ...
  const bumpLocalPreviewSurfaceVersion = useCallback(
    (reason = "unknown") => {
  ...
  const activateConnectedState = useCallback(
    (startedAt) => {
      if (callSyncTimeoutRef.current) {
        clearTimeout(callSyncTimeoutRef.current);
        callSyncTimeoutRef.current = null;
      ...
      startTimer(
        resolvedStartedAt
          ? Math.max(
              0,
              Math.floor(
                (Date.now() - new Date(resolvedStartedAt).getTime()) / 1000,
              ),
            )
          : 0,
        "call_connected",
  ...
  const requestLocalVideoBootstrap = useCallback(
    (reason = "unknown", callType = callTypeRef.current) => {
  ...
  const cleanupAgoraSession = useCallback(
  ...
  const joinAgoraIfNeeded = useCallback(async () => {
    ...
    try {
      ...
      const resolvedChannelName =
        ...
        activePayload?.agora?.channelName;
      if (
        !latestAgoraRef.current?.appId ||
        !latestAgoraRef.current?.token ||
        !resolvedChannelName
      ) {
      ...
      if (backendMarkedActiveRef.current) {
        ...
        setStatusText(
          activeCallType === "video" ? "Connecting video..." : "Connecting...",
        );
      }
    } catch (error) {
      hasJoinedAgoraRef.current = false;
  ...
  const handleSessionEnded = useCallback(
    (eventPayload) => {
      ...
      const normalizedEndReason = String(eventPayload?.endReason || "")
        .trim()
        .toUpperCase();
      const normalizedReasonCode = String(eventPayload?.reasonCode || "")
        .trim()
        .toUpperCase();
  ...
  const reconcileCallerCallState = useCallback(
    async (reason = "poll") => {
      if (
        isDemoSession ||
        isListener ||
      ...
      try {
        ...
        if (
          realtime?.isRejected ||
          ["REJECTED", "CANCELLED", "MISSED", "ENDED"].includes(
            normalizedStatus,
          )
        ) {
          const rejectedReasonCode =
            realtime?.rejectedReasonCode ||
            (normalizedStatus === "REJECTED" ? "CALL_REJECTED" : "");
          const isRejectedCall =
            normalizedStatus === "REJECTED" ||
            rejectedReasonCode === "CALL_REJECTED";
  ...
  useEffect(() => {
    logCallDebug("callScreenInit", {
      ...
      initialStatus:
        runtimePayloadRef.current?.session?.status ||
    ...
    if (isDemoSession) {
      ...
      return () => {
        if (demoConnectTimeoutRef.current)
          clearTimeout(demoConnectTimeoutRef.current);
    ...
    if (!sessionId) {
      Alert.alert(
        "Call unavailable",
        "Unable to start call session right now.",
      );
      navigation.goBack();
      return undefined;
    }
    ...
    const onCallAccepted = async (eventPayload) => {
      ...
      const acceptedCallType = normalizeCallType(
        eventPayload?.callType || callTypeRef.current,
      );
      ...
      setStatusText(
        acceptedCallType === "video" ? "Connecting video..." : "Connecting...",
      );
      try {
        await joinAgoraIfNeeded();
      } catch (error) {
        if (!isUnauthorizedApiError(error))
          Alert.alert(
            ...
            error?.message || "Unable to connect call.",
          );
      }
    };
    const onCallStarted = async (eventPayload) => {
      ...
      backendMarkedActiveRef.current = true;
      callStartedAtRef.current =
        eventPayload?.startedAt ||
        eventPayload?.answeredAt ||
        callStartedAtRef.current;
      setRuntimePayload((prev) =>
        prev
          ? {
              ...
              session: {
                ...
                answeredAt:
                  eventPayload?.answeredAt || prev.session?.answeredAt,
                callType:
      ...
      if (eventPayload?.sessionId === sessionId)
        setLowBalanceWarning(
          eventPayload?.message ||
            "Low balance. Please recharge to avoid disconnection.",
        );
    };
    ...
    return () => {
      ...
      logCallDebug(
        keepCallAlive
          ? "screenCleanupSkippedActiveCall"
          : "screenCleanupProceeding",
        {
          sessionId,
          appState: appStateRef.current,
          callConnected: callConnectedRef.current,
  ...
  const onAcceptIncoming = async () => {
    ...
    if (isDemoSession) {
      ...
      setRuntimePayload({
        session: {
          ...
          listenerId:
            incomingRequest?.requester?.id || host?.listenerId || null,
      ...
      setStatusText(
        demoCallType === "video" ? "Connecting video..." : "Connecting...",
      );
      resolvingIncomingRef.current = false;
      setIsResolvingIncoming(false);
      return;
    ...
    try {
      ...
      setStatusText(
        nextCallType === "video" ? "Connecting video..." : "Connecting...",
      );
      ...
      if (!isUnauthorizedApiError(error))
        Alert.alert(
          ...
          error?.response?.data?.message ||
            error?.message ||
            "Unable to accept this call right now.",
        );
  ...
  const onEndCall = async () => {
    if (isDemoSession) {
      ...
      await endDemoCallSession(
        sessionId,
        isListener ? "LISTENER_ENDED" : "USER_ENDED",
      );
  ...
  const callHeaderSecondaryText = isCallConnected
    ? fmt(elapsedSeconds)
    : statusText;
  const hasRemoteVideoTrack = Number.isFinite(Number(remoteVideoUid));
  ...
  return (
    <LinearGradient colors={theme.gradients.bg} style={styles.container}>
      ...
      <SafeAreaView
        style={[
          styles.safeArea,
          showVideoSurfaces ? styles.safeAreaVideo : null,
        ]}
      ...
      >
        <TouchableOpacity
          style={[
            styles.backBtn,
            {
              marginTop: Math.max(8, insets.top + 2),
              marginLeft: showVideoSurfaces ? 12 : 0,
            },
          ]}
          onPress={minimizeCallScreen}
          activeOpacity={0.85}
        >
          <Ionicons
            name="chevron-back"
        ...
        <View
          pointerEvents="none"
          style={[styles.topCallHeader, { top: Math.max(12, insets.top + 6) }]}
        >
        ...
        <View style={styles.center}>
          {showVideoSurfaces ? (
            <View
              style={[
                styles.videoSurfaceLayer,
                showVideoSurfaces ? styles.videoSurfaceLayerFullscreen : null,
              ]}
            >
              {hasAgoraRtcSurfaceView && hasRemoteVideoTrack ? (
                <RtcSurfaceView
              ...
              !hasRemoteVideoFrame ? (
                <View style={styles.videoFallback}>
                  <Image
                    source={avatar}
                    style={styles.videoFallbackAvatar}
                    resizeMode="cover"
                  />
                  <Text style={styles.videoFallbackText}>
                    {!hasAgoraRtcSurfaceView
                      ...
                      : hasRemoteVideoTrack
                        ? "Remote camera is off or stream is loading..."
        ...
        {callMode === "incoming" ? (
          ...
          >
            <TouchableOpacity
              ...
              activeOpacity={0.88}
              disabled={isResolvingIncoming}
            >
              <MaterialIcons name="call" size={28} color="#FFFFFF" />
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.answerBtn, styles.rejectBtn]}
          ...
          <View
            style={[
              styles.controlsDock,
              { marginBottom: Math.max(18, insets.bottom + 10) },
            ]}
          >
            <TouchableOpacity
            ...
            <TouchableOpacity
              style={[
                styles.controlBtn,
                isSpeakerOn ? styles.controlBtnActive : null,
              ]}
            ...
            {isVideoCall ? (
              <TouchableOpacity
                style={[
                  styles.controlBtn,
                  isCameraEnabled ? styles.controlBtnActive : null,
                ]}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  safeArea: { flex: 1, paddingHorizontal: 16, paddingBottom: 18 },
  safeAreaVideo: { paddingHorizontal: 0, paddingBottom: 0 },
  backBtn: {
    width: 42,
    height: 42,
    borderRadius: 21,
  ...
  glowRing: {
    ...
    height: 204,
    borderRadius: 102,
    backgroundColor: "rgba(209,11,149,0.15)",
